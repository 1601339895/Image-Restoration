...
class MSG_FFN(nn.Module):
    """
    Multi-Scale Gated Feed-Forward Network (MSG-FFN)

    Removed context_emb for better stability.
    Fixed the channel split error by forcing even hidden dimensions.
    """

    def __init__(self, dim, ffn_expansion_factor=2.66, bias=False):
        super(MSG_FFN, self).__init__()

        # 【关键修复】确保 hidden_features 是偶数，保证 split/chunk 绝对安全
        hidden_features = int(dim * ffn_expansion_factor)
        if hidden_features % 2 != 0:
            hidden_features -= 1  # 强制转为偶数

        self.hidden_features = hidden_features
        self.gate_split_dim = hidden_features // 2

        # 1. 升维投影
        self.project_in = nn.Conv2d(dim, hidden_features * 2, kernel_size=1, bias=bias)

        # 2. 内容流 (Content Branch)
        self.dwconv_content = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1,
                                        groups=hidden_features, bias=bias)

        # 3. 多尺度门控流 (Multi-Scale Gate Branch)
        # 分支 A: 局部/高频 (3x3)
        self.dwconv_gate_local = nn.Conv2d(self.gate_split_dim, self.gate_split_dim, kernel_size=3, stride=1, padding=1,
                                           groups=self.gate_split_dim, bias=bias)

        # 分支 B: 全局/低频 (Dilated 3x3, rate=3) -> 感受野 7x7
        self.dwconv_gate_context = nn.Conv2d(self.gate_split_dim, self.gate_split_dim, kernel_size=3, stride=1,
    ...
    def forward(self, x):
        ...
        # 3. 处理 Gate 分支 (多尺度)
        # 使用 chunk(2) 代替 split，且构造函数已保证通道为偶数，不会报错
        g_local, g_context = x_gate.chunk(2, dim=1)

        # 并行处理
        g_local = self.dwconv_gate_local(g_local)
        g_context = self.dwconv_gate_context(g_context)

        # 拼接
        x_gate_processed = torch.cat([g_local, g_context], dim=1)

        # 激活 (GELU)
        x_gate_active = F.gelu(x_gate_processed)

        # 4. 门控乘法
        x = x_content * x_gate_active
...
class DynamicTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, context_dim):
        ...
        self.ffn = MSG_FFN(dim, ffn_expansion_factor, bias)
