...
class DFA_FFN(nn.Module):
    """
    Dynamic Frequency-Aggregation Feed-Forward Network
    创新点:
    1. 降低 Expansion Factor 到 2.0 (省参数)。
    2. 引入 Dynamic Context Modulation (动态利用 Context 信息)。
    3. 引入 Frequency Branch (显式增强高频/边缘信息，针对去雨去噪)。
    """

    def __init__(self, dim, ffn_expansion_factor, bias=False, context_dim=64):
        super(DFA_FFN, self).__init__()

        hidden_features = int(dim * ffn_expansion_factor)

        # 1. 动态上下文投影 (极轻量)
        # 将全局 Context 映射为 Channel 权重，用于调制门控
        self.context_gate = nn.Sequential(
            nn.Linear(context_dim, hidden_features),
            nn.Sigmoid()
        )
        ...
        # 4. [核心创新] 频率感知门控流 (负责筛选特征)
        # 分支A: 标准 3x3 DW (捕捉局部上下文)
        self.dwconv_gate_spatial = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1,
                                             groups=hidden_features, bias=bias)

        # 分支B: 高频增强 (无需学习参数，或者仅极少量参数)
        # 这里使用 "Input - AvgPool(Input)" 来近似高频信息 (High-Pass)
    ...
    def forward(self, x, context_emb):
        """
        x: [B, C, H, W]
        context_emb: [B, context_dim] -> 来自 Context_Perception_Module 的 global_feat
        """
        # 1. 升维
        x_proj = self.project_in(x)
        x_content, x_gate = x_proj.chunk(2, dim=1)

        # 2. 处理 Content 分支
        x_content = self.dwconv_content(x_content)

        # 3. [创新] 处理 Gate 分支 - 频率分解与动态调制
        # (a) 空间门控 (类似于 GDFN)
        gate_spatial = self.dwconv_gate_spatial(x_gate)

        # (b) 高频提取 (High-Frequency Extraction)
        # x_gate 的高频部分 = x_gate - LowFreq(x_gate)
        # 这对于去除雨纹（高频）和保留纹理至关重要
        gate_highfreq = x_gate - self.avg_pool(x_gate)

        # (c) 融合频率信息
        # 简单的加和，让 Gate 既包含空间上下文，又包含锐利的边缘信息
        raw_gate = gate_spatial + gate_highfreq

        # (d) 动态上下文调制 (Dynamic Context Modulation)
        # 根据当前是雨天还是雾天，动态调整通道的激活阈值
        # context_emb: [B, C_ctx] -> [B, Hidden] -> [B, Hidden, 1, 1]
        context_weight = self.context_gate(context_emb).unsqueeze(-1).unsqueeze(-1)

        # 最终门控生成
        final_gate = self.act(raw_gate) * context_weight

        # 4. Gating 操作
        x = x_content * final_gate
...
class DynamicTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, context_dim):
        ...
        self.ffn = DFA_FFN(dim, ffn_expansion_factor, bias, context_dim)
