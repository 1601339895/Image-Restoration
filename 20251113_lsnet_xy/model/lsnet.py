...
if __name__ == "__main__":
    ...
    model = lsnet_t(num_classes=2).to("cuda:0")
    model.eval()

    flops, params = profile(model, (x,))
    # 输出 FLOPs 和参数数量，增加描述文字
    print(f"Model Complexity:")
    print(f"  - FLOPs: {flops / 1e6:.2f} MFlops (billion floating point operations)")
    print(f"  - Parameters: {params / 1e6:.2f} M (million parameters)")
    '''
    Model Complexity:
    - FLOPs: 320.32 MFlops (billion floating point operations)
    - Parameters: 11.02 M (million parameters)
    '''