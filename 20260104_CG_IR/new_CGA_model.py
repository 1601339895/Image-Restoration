...
class Adaptive_Gated_Fusion(nn.Module):
    ...
    def forward(self, f_enc, f_dec):
        ...
        # 2. 计算空间权重 (H, W 维度上的筛选)
        # spatial_logit: [B,1,H,W] ->关注那些像素重要
        spatial_logit = self.spatial_gate(combined)

        # 3. 计算通道权重 (C 维度上的筛选)
        # 哪个通道包含更多噪声？
        # channel_logit:[B,C, 1, 1] ->关注那些通道重要
        ...
        # 广播相加+Sigmoid
        # 利用广播机制将[B,,H,W]和[B,C,1,1]结合
        # 为每个通道生成了独特的空间掩码
        atten_weight = torch.sigmoid(spatial_logit + channel_logit)

        # 5.筛选Encoder特征，
        f_enc_filtered = f_enc * atten_weight  # 用一次乘法，不用2次惩罚
...
class CAFGB(nn.Module):
    ...
    def forward(self, x, global_feat):
        ...
        scale = scale.to(torch.float32)  # 转为 float32
...
class Context_Gated_Attention(nn.Module):
    """
    Context-Gated Multi-Head Attention (CGA)
    1. Context-Adaptive Temperature (Log-Space):
       Controls attention sharpness by modulating scaled dot-product in log domain.
    2. Element-wise Attention Output Gating:
       Context generates per-head, per-channel gates applied on attention output
    """
    ...
    def forward(self, x, context_emb):
        ...
        # -------------------------------------------------------
        # Step 4: Element-wise Gated Attention Output (Qwen3-style)
        # -------------------------------------------------------
        out = out * gate_score
